...
<div class="card p-4">
    <h2 class="text-center mb-4">Редактировать учителя</h2>
    ...
    <form method="POST" action="/admin/teachers/update/{{ $teacher->id }}" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <textarea name="subjects" class="form-control" rows="4" required>{{ $teacher->subjects }}</textarea>
        </div>

        @if($teacher->photo)
        <div class="mb-3">
            <label class="form-label fw-bold">Текущее фото</label>
            <div>
                <img src="{{ asset('storage/'.$teacher->photo) }}" style="max-width: 200px; border-radius: 8px; border: 1px solid #ddd; padding: 5px;">
            </div>
        </div>
        @endif

        <div class="mb-3">
            <label class="form-label fw-bold">Новое фото (оставьте пустым, если не хотите менять)</label>
        ...
        <div class="d-flex gap-2">
            <button type="submit" class="btn btn-primary-admin">Обновить</button>
