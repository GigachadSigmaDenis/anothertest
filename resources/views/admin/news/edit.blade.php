...
<div class="card p-4">
    <h2 class="text-center mb-4">Редактировать новость</h2>
    ...
    <form method="POST" action="/admin/news/update/{{ $news->id }}" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <input type="datetime-local" name="published_at" class="form-control" 
                   value="{{ $news->published_at ? \Carbon\Carbon::parse($news->published_at)->format('Y-m-d\TH:i') : '' }}" required>
        </div>

        <div class="mb-3">
            <label class="form-label fw-bold">Текст новости</label>
            <textarea name="content" id="editor" class="form-control" rows="12" placeholder="Введите текст новости...">{{ $news->content }}</textarea>
        </div>
        ...
        <div class="mb-3">
            ...
            <div>
                <img src="{{ asset('storage/'.$news->image) }}" style="max-width: 200px; border-radius: 8px; border: 1px solid #ddd; padding: 5px;">
            </div>
        </div>
        @endif

        <div class="mb-3">
            <label class="form-label fw-bold">Новое изображение (оставьте пустым, если не хотите менять)</label>
